...
export default class SubregionListScene extends Component {
    ...
    componentDidMount(){
        ...
        getArea('subregion', region_id)
            .then((msg) => {
                this.setState({subregionList: DrinkDetailDataUtils.requestAreaData(msg)})
            }).catch((error)=>{
                console.warn(error)
        });
    }
    ...
    renderList(){
        let AreaList = this.state.subregionList.map((item) => {
            return(
                <View style={styles.container}>
                    <TouchableOpacity
                        activeOpacity={0.8}
                        onPress={() =>{
                            this.props.navigation.navigate('AreaListScene',{region_id:item.id})
                        }}
                    >
