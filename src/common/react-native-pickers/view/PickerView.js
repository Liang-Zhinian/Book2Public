...
class PickerView extends BaseComponent {
    ...
    constructor(props) {
        ...
        for (let i = 0; i < length; i++) {
            this.colorPath.push(new Animated.Value(i == (this.props.selectedIndex + 1) ? 1 : 0));
        }
        ...
        this.maxTop = 0;
        this.maxBottom = -props.itemHeight * (list.length - 3);
        ...
        //这里固定在屏幕底部，所以直接写死touch区域即可。否则 可使用UIManager.measureInWindow 来计算具体的touch区域
        ...
        if (nextProps) {
            ...
            if (listChange || indexChange) {
                ...
                if (listChange) {
                    ...
                    let length = list.length;
                    for (let i = 0; i < length; i++) {
                        this.colorPath.push(new Animated.Value(i == (nextProps.selectedIndex + 1) ? 1 : 0));
                    }
                }
                nextState.list = list;
                ...
                this.maxTop = 0;
                this.maxBottom = -this.props.itemHeight * (list.length - 3);
                return true;
            }
        }
        return false;
    ...
    onPanResponderEnd(evt, gestureState) {
        ...
        if (actionTime < 300 && Math.abs(gestureState.vy) < 0.1) {
            ...
            let toValue = this._previousTop + gestureState.vy * this.props.itemHeight * 1;
            let number = Math.round(toValue / this.props.itemHeight);
            toValue = this.props.itemHeight * number;
            if (toValue > 0) {
                toValue = Math.min(toValue, this.maxTop);
    ...
    render() {
        return <View style={{
            width: this.props.itemWidth, height: this.props.itemHeight * 3 + this.getSize(15),
        }}>
            <View
